...
export const Modal = ({handleModalClose, title, sectionInfo, nodeData, lesson}) => {    
    ...
    let started = nodeData.targetNode.started

    //console.log(nodeData.targetNode.pages[nodeData.targetNode.pages.length - 1].content);
            ...
            {currentPage === 0 ? (<>
                ...
                <div className={`flex`}>
                    ...
                    <div className={`flex flex-col items-center justify-center p-4 ml-5 w-3/4`}>
                        {started ? <button className={`p-2 mb-5 bg-green-700 text-white w-full`} onClick={handleNextPage}>Continue</button> : <button className={`p-2 mb-5 bg-green-700 text-white w-full`} onClick={handleStartSection}>Start Section</button>}
